...
const PopularTV: ForwardRefRenderFunction<unknown, DiscoverTab> = (
  props,
  ref
) => {
  const { setActiveRow } = props;
  const { data, isLoading } = api.discover.popularTV.useQuery({ page: 1 });
...
export default memo(forwardRef(PopularTV));
